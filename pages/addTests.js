...
const {WEB_API_URL, API_NEW_TEST} = Keys;
...
const AddTests = () => {
  ...
  const onAdd = async (data) => {
    console.log("in Add Tests");
    const ADD_TEST_URL = `${WEB_API_URL}${API_NEW_TEST}${route.params.id}`;
    console.log('data:', data)

    await axios.post(ADD_TEST_URL,{
      ...
      <ScrollView style={{ width: '100%' }}>
        ...
        render={({field:{onChange, value}, fieldState:{error}}) => (
          <>
            <TextInput style = {[styles.Input,{borderColor: error ? 'red' : 'black'}]}
                         placeholder = "" onChangeText={onChange}
                         value={value}/>
        ...
        render={({field:{onChange, value}, fieldState:{error}}) => (
          <>
           ...
           name="heartbeat"
           ></Controller>
        <Text style={styles.heading}>Admission Date</Text>
        <View>
        <Controller defaultValue="" control={control}
        rules={{required:true}}
        render={({field:{onChange, value}, fieldState:{error}}) => (
